...
class BaseSkill extends Component<Props, State> {
	...
	renderInput = (input: Input, i: number) => (
		<div key={i}>
			<p className={style.title}>{input.title}</p>
			{input.type === 'text' && (
				<input
	...
	renderExamples = (ex: Example, i: number) => {
		...
		return (
			<li
				// type="button"
				className={cn(checker && style.active)}
			...
			<div className={style.container}>
				...
				<div className={style.inputArea}>
					<form className={style.inputs} onSubmit={this.onFormSubmit}>
						<div className={style.examplesQuestion}>
							<div className={style.examples}>
								...
								<div style={{ marginTop: '58px' }}>
									{pathName === '/ru/ner' && (
										<div>
											<div className={style.title}>Classes</div>
								...
								<div>
									{answers && (
										<div
											...
											id='answers'
											ref={this.answersRef}>
											<p>{this.lang !== 'ru' ? 'Result' : 'Результат'}</p>
